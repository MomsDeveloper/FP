module NumberLetterTail exposing (numberLetterTail)
...
numberLetterTail : Int -> Int
numberLetterTail n =
    let
        numberToWordsLength =
            String.length << numberToWords

        foldHelper currentLimit currentSum =
            if currentLimit == 0 then
                currentSum

            else
                foldHelper (currentLimit - 1) (currentSum + numberToWordsLength currentLimit)
    in
    foldHelper n 0
